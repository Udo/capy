...
static int add_section(struct macho *mo, struct segment_command_64 **_seg,
					   char *name) {
				...
				if (for_code) {
					if (attr->plt_offset == -1) {
						...
						put_elf_reloc(s1->symtab, mo->stubs,
									  attr->plt_offset + 2, R_X86_64_GOTPCREL,
									  sym_index);
					}
...
static void convert_symbol(TCCState *s1, struct macho *mo,
						   struct nlist_64 *pn) {
...
static void create_symtab(TCCState *s1, struct macho *mo) {
	...
	mo->symtab =
		new_section(s1, "LESYMTAB", SHT_LINKEDIT, SHF_ALLOC | SHF_WRITE);
	mo->strtab =
		new_section(s1, "LESTRTAB", SHT_LINKEDIT, SHF_ALLOC | SHF_WRITE);
...
static void collect_sections(TCCState *s1, struct macho *mo) {
	...
	numsec = 0;
	mo->elfsectomacho =
		tcc_mallocz(sizeof(*mo->elfsectomacho) * s1->nb_sections);
...
ST_FUNC int macho_load_dll(TCCState *s1, int fd, const char *filename,
						   int lev) {
...
again:
	...
	if (fh.magic == FAT_MAGIC || fh.magic == FAT_CIGAM) {
		struct fat_arch *fa =
			load_data(fd, sizeof(fh), fh.nfat_arch * sizeof(*fa));
	...
	buf2 =
		load_data(fd, machofs + sizeof(struct mach_header_64), mh.sizeofcmds);
	for (i = 0, lc = buf2; i < mh.ncmds; i++) {
		...
		case LC_SYMTAB: {
			...
			nsyms = sc->nsyms;
			symtab =
				load_data(fd, machofs + sc->symoff, nsyms * sizeof(*symtab));
			strsize = sc->strsize;
		...
		case LC_REEXPORT_DYLIB: {
			...
			dprintf(" REEXPORT %s\n", name);
			if (subfd < 0)
				tcc_warning("can't open %s (reexported from %s)", name,
							filename);
			else {
	...
	for (i = iextdef; i < iextdef + nextdef; i++) {
		...
		dprintf("%5d: %3d %3d 0x%04x 0x%016llx %s\n", i, sym->n_type,
				sym->n_sect, sym->n_desc, sym->n_value, strtab + sym->n_strx);
