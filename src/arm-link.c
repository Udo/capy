	...
	if (p < p_end) {
		...
		while (p < p_end) {
			...
			if (read32le(p) == 0x46c04778) /* PLT Thumb stub present */
				p += 4;
			write32le(p, 0xe28fc600 |
							 ((off >> 20) & 0xff)); // add ip, pc, #0xNN00000
			write32le(p + 4,
					  0xe28cca00 |
						  ((off >> 12) & 0xff)); // add ip, ip, #0xNN000
			write32le(p + 8,
					  0xe5bcf000 | (off & 0xfff)); // ldr pc, [ip, #0xNNN]!
			p += 12;
		}
	}
}
...
void relocate(TCCState *s1, ElfW_Rel *rel, int type, unsigned char *ptr,
	...
	case R_ARM_THM_JUMP24: {
		...
		plt = s1->plt;
		to_plt =
			(val >= plt->sh_addr) && (val < plt->sh_addr + plt->data_offset);
		...
		if (!to_thumb && !to_plt && !is_call) {
			...
			index =
				put_elf_sym(symtab_section, text->data_offset + 1, sym->st_size,
							sym->st_info, 0, sym->st_shndx, buf);
			to_thumb = 1;
			val = text->data_offset + 1;
			rel->r_info = ELFW(R_INFO)(index, type);
			/* Create a thumb stub function to switch to ARM mode */
			put_elf_reloc(symtab_section, text, text->data_offset + 4,
						  R_ARM_JUMP24, sym_index);
		...
		(*(uint16_t *)(ptr + 2)) = (uint16_t)((lo & 0xc000) | (j1 << 13) |
											  blx_bit | (j2 << 11) | imm11);
	}
		return;
	case R_ARM_MOVT_ABS:
