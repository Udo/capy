...
tokenizer_state* tokenize(string* content, char* filename)
{
	...
	for (u64 i = 0; i < content->length; i++)
	{
		...
		// todo: we might want to see if there is a unicode code point in the pipeline in case we want to react to a unicode glyph
		if(pmode == '.') // 'normal' mode
		{
			if(token_current->type == 0)
			{
				// if the current token is empty, fill its type
				token_current->type = ctype;
			}
			else if(ctype != token_current->type || ctype == 'P')
			{
				// do we need to start a new token?
				token_current = new_token(ctype, token_current, i);
			}
			if(c == '"' || c == '\'')
			{
				// our two quote characters
				pmode = c;
			...
			else if (c == '<' && lastctype == 'P')
			{
				...
				token_current->type = 'Q'; // for now, maybe we'll change it to H later
				xml_level = 1;
				hblock_count += 1;
