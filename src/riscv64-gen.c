...
static int is_freg(int r) { return r >= 8 && r < 16; }
...
static void EIu(uint32_t opcode, uint32_t func3, uint32_t rd, uint32_t rs1,
				uint32_t imm) {
	o(opcode | (func3 << 12) | (rd << 7) | (rs1 << 15) | (imm << 20));
}

static void ER(uint32_t opcode, uint32_t func3, uint32_t rd, uint32_t rs1,
			   uint32_t rs2, uint32_t func7) {
	o(opcode | func3 << 12 | rd << 7 | rs1 << 15 | rs2 << 20 | func7 << 25);
}

static void EI(uint32_t opcode, uint32_t func3, uint32_t rd, uint32_t rs1,
			   uint32_t imm) {
	assert(!((imm + (1 << 11)) >> 12));
	EIu(opcode, func3, rd, rs1, imm);
}

static void ES(uint32_t opcode, uint32_t func3, uint32_t rs1, uint32_t rs2,
			   uint32_t imm) {
	assert(!((imm + (1 << 11)) >> 12));
...
static int load_symofs(int r, SValue *sv, int forstore) {
	...
	if (sv->r & VT_SYM) {
		...
		if (sv->sym->type.t & VT_STATIC) { // XXX do this per linker relax
			greloca(cur_text_section, sv->sym, ind, R_RISCV_PCREL_HI20,
					sv->c.i);
			sv->c.i = 0;
		} else {
		...
		greloca(cur_text_section, &label, ind,
				doload || !forstore ? R_RISCV_PCREL_LO12_I
		...
		rr = 8; // s0
		if (fc != sv->c.i)
			tcc_error("unimp: store(giant local off) (0x%llx)",
					  (long long)sv->c.i);
		if (((unsigned)fc + (1 << 11)) >> 12) {
			rr = is_ireg(r) ? ireg(r) : 5; // t0
			o(0x37 | (rr << 7) |
			  ((0x800 + fc) & 0xfffff000)); // lui RR, upper(fc)
	...
	if (fr & VT_LVAL) {
		...
		if (fc != sv->c.i) {
			...
			if (si != 0) {
				...
				EI(0x13, 0, rr, rr,
				   (int)pi << 20 >> 20); // addi RR, RR, lo(up(fc))
				EI(0x13, 1, rr, rr, 12); // slli RR, RR, 12
				EI(0x13, 0, rr, rr,
				   (fc + (1 << 19)) >> 20); // addi RR, RR, up(lo(fc))
				EI(0x13, 1, rr, rr, 12);	// slli RR, RR, 12
				fc = fc << 12 >> 12;
		...
		if (fc || (rr != rb) || do32bit || (fr & VT_SYM))
			EI(0x13 | do32bit, 0, rr, rb,
			   fc << 20 >> 20); // addi[w] R, x0|R, FC
		if (zext) {
...
static void gcall_or_jmp(int docall) {
	...
	if ((vtop->r & (VT_VALMASK | VT_LVAL)) == VT_CONST &&
		...
		greloca(cur_text_section, vtop->sym, ind, R_RISCV_CALL_PLT,
				(int)vtop->c.i);
	...
	if ((type->t & VT_BTYPE) == VT_STRUCT) {
		...
		if (type->ref->c < 0 || type->ref->c > 2)
			...
			if (rc[0] > 2 || (rc[0] == 2 && type->ref->c > 1))
				...
				rc[++rc[0]] = RC_FLOAT;
				fieldofs[rc[0]] =
					((ofs + sz) << 4) | (type->ref->type.t & VT_BTYPE);
		...
		if (areg[prc[1] - 1] >= 8 ||
			...
			for (i = 0; i < regcount; i++) {
				if (areg[prc[1 + i] - 1] >= 8) {
					...
					ES(0x23, 3, 8, 10 + areg[0]++,
					   loc + i * 8); // sd aX, loc(s0) // XXX
				}
			}
		}
...
ST_FUNC void gfunc_epilog(void) {
	...
	if (v >= (1 << 11)) {
		d = 16;
		o(0x37 | (5 << 7) |
		  ((0x800 + (v - 16)) & 0xfffff000));	 // lui t0, upper(v)
...
ST_FUNC void gen_opi(int op) { gen_opil(op, 0); }

ST_FUNC void gen_opl(int op) { gen_opil(op, 1); }
...
ST_FUNC void gen_cvt_ftof(int dt) {
	...
	if (dt == VT_LDOUBLE || st == VT_LDOUBLE) {
		...
		if (dt == VT_LDOUBLE)
			...
			assert(vtop->r2 < 7);
			if (vtop->r2 != 1 + vtop->r) {
				EI(0x13, 0, ireg(vtop->r) + 1, ireg(vtop->r2),
				   0); // mv Ra+1, RR2
				vtop->r2 = 1 + vtop->r;
			}
		}
