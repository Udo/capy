...
LIBTCCAPI int tcc_relocate(TCCState *s1, void *ptr) {
...
#ifdef HAVE_SELINUX
	{
		...
		dynarray_add(&s1->runtime_mem, &s1->nb_runtime_mem,
					 (void *)(addr_t)size);
...
static void set_pages_executable(TCCState *s1, void *ptr,
...
static addr_t rt_printline(rt_context *rc, addr_t wanted_pc, const char *msg,
						   const char *skip) {
	char func_name[128];
...
ST_FUNC void *dlopen(const char *filename, int flag) { return NULL; }

ST_FUNC void dlclose(void *p) {}

ST_FUNC const char *dlerror(void) { return "error"; }
