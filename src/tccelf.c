	...
	for (i = 1; i < s1->nb_sections; i++) {
		...
		if (sr->sh_type == SHT_RELX && sr->link == s) {
			...
			for (; rel < rel_end; ++rel) {
				...
				// if (n < 0) tcc_error("internal: invalid symbol index in
				// relocation");
...
static Section *find_section_create(TCCState *s1, const char *name,
									int create) {
	Section *sec;
	int i;
	for (i = 1; i < s1->nb_sections; i++) {
...
ST_FUNC int put_elf_sym(Section *s, addr_t value, unsigned long size, int info,
	...
	if (hs) {
		...
		if (ELFW(ST_BIND)(info) != STB_LOCAL) {
			...
			nbuckets = base[0];
			h = elf_hash((unsigned char *)s->link->data + name_offset) %
				nbuckets;
			*ptr = base[2 + h];
			base[2 + h] = sym_index;
			base[1]++;
		...
		for (i = nb_sym_versions; i-- > 0;) {
			...
			do {
				...
				if (sv->out_index > 0) {
					...
					vna->vna_hash =
						elf_hash((const unsigned char *)sv->version);
					vna->vna_flags = 0;
					vna->vna_other = sv->out_index;
					sv->out_index = -2;
					vna->vna_name =
						put_elf_str(verneed_section->link, sv->version);
...
ST_FUNC int set_elf_sym(Section *s, addr_t value, unsigned long size, int info,
	...
	if (sym_bind != STB_LOCAL) {
		...
		if (esym->st_shndx != SHN_UNDEF) {
			...
			esym->st_other =
				(esym->st_other & ~ELFW(ST_VISIBILITY)(-1)) | new_vis;
	...
	do_def:
		sym_index =
			put_elf_sym(s, value, size, ELFW(ST_INFO)(sym_bind, sym_type),
						other, shndx, name);
	}
	return sym_index;
}
...
ST_FUNC void squeeze_multi_relocs(Section *s, size_t oldrelocoffset) {
	...
	for (a = oldrelocoffset + sizeof(*r); a < sr->data_offset;
		 a += sizeof(*r)) {
		...
		if (sh_num == SHN_UNDEF) {
		   ...
		   value is zero */
			...
			if (sym_bind == STB_WEAK)
				...
				tcc_error_noabort("undefined symbol [ELF] '%s'", name);
...
static int prepare_dynamic_rel(TCCState *s1, Section *sr) {
	...
	(defined(TCC_TARGET_MACHO) && defined TCC_IS_NATIVE)
...
ST_FUNC void resolve_common_syms(TCCState *s1) {
	...
	for_each_elem(symtab_section, 1, sym, ElfW(Sym)) {
		if (sym->st_shndx == SHN_COMMON) {
			...
			sym->st_value =
				section_add(bss_section, sym->st_size, sym->st_value);
		...
		if (sym->st_shndx == SHN_UNDEF) {
			...
			if (sym_index) {
				...
				if ((type == STT_FUNC) || (type == STT_GNU_IFUNC)) {
					 ...
					 * address */
					int dynindex = put_elf_sym(
						s1->dynsym, 0, esym->st_size,
					...
					if (ELFW(ST_BIND)(esym->st_info) == STB_WEAK) {
						for_each_elem(s1->dynsymtab_section, 1, dynsym,
									  ElfW(Sym)) {
							if ((dynsym->st_value == esym->st_value) &&
								(ELFW(ST_BIND)(dynsym->st_info) ==
								 STB_GLOBAL)) {
								char *dynname =
									(char *)s1->dynsymtab_section->link->data +
									dynsym->st_name;
					...
					put_elf_reloc(s1->dynsym, bss_section, offset, R_COPY,
								  index);
					offset += esym->st_size;
				...
				if (ELFW(ST_BIND)(sym->st_info) == STB_WEAK ||
					...
					tcc_error_noabort("undefined symbol [ELF executable] '%s'", name);
				}
			}
	...
	for (i = 1; i < s1->nb_sections; i++) {
		...
		if (file_type == TCC_OUTPUT_DLL && s->sh_type == SHT_RELX &&
			...
			if ((s1->do_debug && s->sh_type != SHT_RELX) ||
				file_type == TCC_OUTPUT_OBJ || (s->sh_flags & SHF_ALLOC) ||
				i == (s1->nb_sections - 1)
...
static int layout_sections(TCCState *s1, ElfW(Phdr) * phdr, int phnum,
		   ...
		   symbol tables, relocations, progbits, nobits */
			...
			for (k = 0; k < 5; k++) {
				for (i = 1; i < s1->nb_sections; i++) {
					...
					tmp = addr;
					addr =
						(addr + s->sh_addralign - 1) & ~(s->sh_addralign - 1);
	...
	for (i = 1; i < s1->nb_sections; i++) {
		...
		file_offset =
			(file_offset + s->sh_addralign - 1) & ~(s->sh_addralign - 1);
		s->sh_offset = file_offset;
		if (s->sh_type != SHT_NOBITS)
			file_offset += s->sh_size;
	}
...
static void tcc_output_elf(TCCState *s1, FILE *f, int phnum, ElfW(Phdr) * phdr,
	...
	/* FIXME: should set only for freebsd _target_, but we exclude only PE
	 * target
	 */
...
static int tcc_write_elf_file(TCCState *s1, const char *filename, int phnum,
							  ElfW(Phdr) * phdr, int file_offset,
							  int *sec_order) {
...
static int elf_output_file(TCCState *s1, const char *filename) {
	...
	if (file_type != TCC_OUTPUT_OBJ) {
		...
		if (!s1->static_link) {
			...
			/* add dynamic section */
			dynamic =
				new_section(s1, ".dynamic", SHT_DYNAMIC, SHF_ALLOC | SHF_WRITE);
			dynamic->link = dynstr;
...
ST_FUNC int tcc_load_object_file(TCCState *s1, int fd,
	...
	for (i = 1; i < nb_syms; i++, sym++) {
		...
		name = strtab + sym->st_name;
		sym_index =
			set_elf_sym(symtab_section, sym->st_value, sym->st_size,
	...
	for (i = 1; i < ehdr.e_shnum; i++) {
		...
		case SHT_RELX:
			...
			for_each_elem(s, (offset / sizeof(*rel)), rel, ElfW_Rel) {
				...
				invalid_reloc:
					tcc_error_noabort(
						"Invalid relocation entry [%2d] '%s' @ %.8x", i,
...
static void set_ver_to_ver(TCCState *s1, int *n, int **lv, int i, char *lib,
	...
	if ((*lv)[i] == -1) {
		...
		if (v == nb_sym_versions) {
			sym_versions =
				tcc_realloc(sym_versions, (v + 1) * sizeof(*sym_versions));
	...
	if (v->versym && v->verneed) {
		...
		do {
			...
			for (i = 0; i < vneed->vn_cnt; i++) {
				...
				vernaux =
					(ElfW(Vernaux) *)(((char *)vernaux) + vernaux->vna_next);
			}
			next = vneed->vn_next;
	...
	for (i = 0; i < v->nb_local_ver; i++) {
		if (v->local_ver[i] > 0) {
			printf("%d: lib: %s, version %s\n", i,
				   sym_versions[v->local_ver[i]].lib,
		...
		if (!strcmp(filename, "AS_NEEDED")) {
			...
			if (!as_needed) {
				...
				if (ret)
					goto lib_parse_error;
				if (group) {
					/* Add the filename *and* the libname to avoid future
					 * conversions */
