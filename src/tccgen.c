...
static void patch_type(Sym *sym, CType *type)
{
    ...
    if (!(type->t & VT_EXTERN) || IS_ENUM_VAL(sym->type.t)) {
        if (!(sym->type.t & VT_EXTERN))
        {
			...
			tcc_error("type error, cannot redefine "ANSI_FG_CYAN "%s" ANSI_RESET "(%s) to "
				ANSI_FG_CYAN "%s" ANSI_RESET "(%s)",
    ...
    if (!is_compatible_types(&sym->type, type)) {
        tcc_error("incompatible types for redefinition of '" ANSI_FG_CYAN "%s" ANSI_RESET "'",
                  get_tok_str(sym->v, NULL));
        ...
        if ((type->t & VT_STATIC) && !static_proto
            ...
            tcc_warning("static storage ignored for redefinition of '" ANSI_FG_CYAN "%s" ANSI_RESET "'",
                get_tok_str(sym->v, NULL));
        ...
        if ((type->t ^ sym->type.t) & VT_STATIC)
            tcc_warning("storage mismatch for redefinition of '" ANSI_FG_CYAN "%s" ANSI_RESET "'",
                get_tok_str(sym->v, NULL));
    }
}
...
static void patch_storage(Sym *sym, AttributeDef *ad, CType *type)
{
    ...
    if (sym->a.dllimport != ad->a.dllimport)
        tcc_error("incompatible dll linkage for redefinition of '" ANSI_FG_CYAN "%s" ANSI_RESET "'",
            get_tok_str(sym->v, NULL));
#endif
            ...
            if (pbt1 != VT_VOID && pbt2 != VT_VOID
                ...
                if (op != '?' && !TOK_ISCOND(op))
                  ...
                  type_incompatibility_warning(type1, type2,
                    op == '?'
                     ? "pointer type mismatch in conditional expression ('" ANSI_FG_CYAN "%s" ANSI_RESET "' and '" ANSI_FG_CYAN "%s" ANSI_RESET "')"
                     : "pointer type mismatch in comparison('" ANSI_FG_CYAN "%s" ANSI_RESET "' and '" ANSI_FG_CYAN "%s" ANSI_RESET "')");
            }
            if (op == '?') {
...
static void cast_error(CType *st, CType *dt)
{
    type_incompatibility_error(st, dt, "cannot convert '" ANSI_FG_CYAN "%s" ANSI_RESET "' to '" ANSI_FG_CYAN "%s" ANSI_RESET "'");
}
...
static void verify_assign_cast(CType *dt)
{
    ...
    case VT_PTR:
        ...
        if (is_basic_type(sbt)) {
            ...
            tcc_error("cannot assign <%s> = <%s> without a cast",
				tbuf2, tbuf1);
            break;
        }
	...
	case TOK_CLEANUP2:
	{
	    ...
	    if (!s) {
	      ...
	      s = external_global_sym(tok, &func_old_type);
                ...
                tcc_error("'" ANSI_FG_CYAN "%s" ANSI_RESET "' is not callable", get_tok_str(tok, &tokc));
	    ad->cleanup_func = s;
	    next();
            skip(')');
	    break;
    ...
    while(1) {
        ...
        basic_type1:
            if (u == VT_SHORT || u == VT_LONG) {
                ...
                {
					tmbt: tcc_error("cannot redefine type/def for <%s>", get_tok_str(p_tok, NULL));
                }
                st = u;
            } else {
  ...
  abstract:
	...
	*v = 0;
	...
	/*
	{
		char tbuf1[256];
		type_to_str(tbuf1, sizeof(tbuf1), type, NULL);
		printf("DEBUG: type %s sym=%s t=%i\n", tbuf1, get_tok_str(v,0), type_to_id(type));
	}
	*/
...
ST_FUNC void unary(void)
{
 ...
 push_tokc:
	...
	if (!str) {
	    ...
	    tcc_error("type '" ANSI_FG_CYAN "%s" ANSI_RESET "' does not match any association", buf);
	}
	begin_macro(str, 1);
	next();
	expr_eq();
...
special_math_val:
    ...
    tok_identifier:
        ...
        if (!s || IS_ASM_SYM(s)) {
            ...
            if (tok != '(')
            {
                tcc_error("'" ANSI_FG_CYAN "%s" ANSI_RESET "' is undeclared", name);
            }
	...
	c = condition_3way();
        ...
        if (!combine_types(&type, &sv, vtop, '?'))
          type_incompatibility_error(&sv.type, &vtop->type,
            "type mismatch in conditional expression ('" ANSI_FG_CYAN "%s" ANSI_RESET "' and '" ANSI_FG_CYAN "%s" ANSI_RESET "')");
...
static void check_func_return(void)
{
    ...
    if (!strcmp (funcname, "main")
        ...
        tcc_warning("function might return no value: '" ANSI_FG_CYAN "%s" ANSI_RESET "'", funcname);
    }
}
	    ...
	    next();
            ...
            if (s) {
                if (s->r == LABEL_DEFINED)
                    tcc_error("duplicate label '" ANSI_FG_CYAN "%s" ANSI_RESET "'", get_tok_str(s->v, NULL));
                s->r = LABEL_DEFINED;
		if (s->next) {
		...
		while (1) { /* iterate thru each declaration */
			...
			if ((type.t & VT_BTYPE) == VT_FUNC) {
				...
				if (ad.a.tag == TAG_HASH_FN) {
					 ...
					 * - so we can later compare the type signature at calling sites
					 * - make a decision about what to do with the return type
					 *   - maybe ignore, make it not part of the signature?
					 *   - or try to infer the return type from the calling site?
					 * - we should convenience-match basic numeric types as long as information is not lost
					 * - if no capy function matches, we should also look whether there's a C function that does
					*/
					/*
			...
			if (ad.a.dllimport || ad.a.dllexport) {
				...
				if (type.t & VT_TYPEDEF) {
					tcc_warning("'" ANSI_FG_CYAN "%s" ANSI_RESET "' attribute ignored for typedef",
			...
			if (tok == '{') {
				...
				while ((sym = sym->next) != NULL) {
					if (!(sym->v & ~SYM_FIELD))
						tcc_error("identifier expected but '" ANSI_FG_CYAN "%s" ANSI_RESET "' found", get_tok_str(sym->v, 0));
				...
				if (l == VT_CMP) {
					...
					for (sym = func_sym->next; sym; sym = sym->next)
						if ((sym->v & ~SYM_FIELD) == v)
							goto found;
					tcc_error("declaration for parameter '" ANSI_FG_CYAN "%s" ANSI_RESET "' but no such parameter",
								get_tok_str(v, NULL));
					found:
					if (type.t & VT_STORAGE) /* 'register' is okay */
						tcc_error("storage class specified for '" ANSI_FG_CYAN "%s" ANSI_RESET "'", get_tok_str(v, NULL));
					if (sym->type.t != VT_VOID)
						tcc_error("redefinition of parameter '" ANSI_FG_CYAN "%s" ANSI_RESET "'", get_tok_str(v, NULL));
					...
					if (sym && sym->sym_scope == local_scope) {
						...
						tcc_error("incompatible redefinition of '" ANSI_FG_CYAN "%s" ANSI_RESET "'", get_tok_str(v, NULL));
						sym->type = type;
					} else {
