...
static const unsigned char tok_two_chars[] =
	...
	*/
	{'<', '=', TOK_LE,		'>', '=', TOK_GE,		 '!', '=', TOK_NE,
	 ...
	 '-', '>', TOK_ARROW,	'<', '>', TOK_ISTR_S,	 '/', '>', TOK_ISTR_E,
	 '.', '.', TOK_TWODOTS, '#', '#', TOK_TWOSHARPS, 0};
...
static TokenSym *tok_alloc_new(TokenSym **pts, const char *str, int len) {
	...
	ts = tal_realloc(toksym_alloc, 0,
					 sizeof(TokenSym) + len +
						 64); // we're padding this a little in case we need to
							  // rewrite an identifier later
	table_ident[i] = ts;
	ts->tok = tok_ident++;
	ts->sym_define = NULL;
	ts->sym_label = NULL;
...
static uint8_t *parse_ii_string(uint8_t *p, CString *str) {
	int c;
	p++;
	for (;;) {
		c = *p;
		if (c == '<' && (*(p + 1) == '/') && (*(p + 2) == '>')) {
			...
			unterminated_string:
				...
				tcc_error("missing string termination </>");
			} else if (c == '\\') {
				/* escape : just skip \[\r]\n */
				PEEKC_EOB(c, p);
				if (c == '\n') {
...
static int macro_is_equal(const int *a, const int *b) {
	...
	while (*a && *b) {
		/* first time preallocate macro_equal_buf, next time only reset position
		 * to start */
			...
			if (tok != ')') {
				if (tok == TOK_ASM_push) {
					...
					if (s1->pack_stack_ptr >=
						s1->pack_stack + PACK_STACK_SIZE - 1)
						goto stk_error;
					s1->pack_stack_ptr++;
					skip(',');
				}
...
static void parse_escape_string(CString *outstr, const uint8_t *buf,
	...
	for (;;) {
		...
		invalid_utf8_sequence:
			tcc_warning("ill-formed UTF-8 subsequence starting with: \'\\x%x\'",
						c);
			c = 0xFFFD;
			p += skip;
			goto add_char_nonext;
...
static void parse_istring(const char *s, int len) { tok = TOK_STR; }
...
redo:
	...
	} else if (t == TOK_ISTR_S) {
		if (parse_flags & PARSE_FLAG_TOK_STR) {
			parse_istring((char *)tokc.str.data, tokc.str.size - 1);
		}
	}
}
...
static void tcc_predefs(CString *cstr) {
	...
	//"#include <tcc_predefs.h>\n"
		...
		"#define __builtin_va_arg(ap,t) "
		"((sizeof(t)>8||(sizeof(t)&(sizeof(t)-1)))?**(t**)((ap+=8)-8):*(t*)(("
		"ap+="
		"8)-8))\n"
#endif
		...
		"(((unsigned)addr+_tcc_alignof(type)-1)&~(_tcc_alignof(type)-1))\n"
		"#define __builtin_va_start(ap,last) "
		"(ap=((char*)&(last))+((sizeof(last)+3)&~3))\n"
		"#define __builtin_va_arg(ap,type) "
		"(ap=(void*)((_tcc_align(ap,type)+sizeof(type)+3)&~3),*(type*)(ap-(("
		"sizeof(type)+3)&~3)))\n"
		...
		"#define _tcc_align(addr,type) (((unsigned "
		"long)addr+__alignof__(type)-1)&-(__alignof__(type)))\n"
		"#define __builtin_va_arg(ap,type) "
		"(*(sizeof(type)>(2*__va_reg_size)?*(type**)((ap+=__va_reg_size)-__va_"
		"reg_size):(ap=(va_list)(_tcc_align(ap,type)+(sizeof(type)+__va_reg_"
		"size-"
		"1)&-__va_reg_size),(type*)(ap-((sizeof(type)+__va_reg_size-1)&-__va_"
		"reg_"
		"size)))))\n"
