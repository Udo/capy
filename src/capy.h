...
struct string {
	...
	// in that case we move the 'bytes' pointer to an offset, whereas the
	// allocated memory block pointer remains safely stored in 'backing_store'.
	// this means we defer any memcpy to events that need to do string_recap()
	u8 *backing_store;
};
...
struct string *string_replace(struct string *src, struct string *find,
							  struct string *rwith);
struct string *string_ireplace(struct string *src, struct string *find,
							   struct string *rwith);
...
void list_insert_after(struct list *l, struct list_item *after,
					   void *item_data);
...
struct hashmap_item *hashmap_next_entry(struct hashmap *h,
...
s32 string_ipos(struct string *src, struct string *find, s32 from_pos) {
	...
	found_chars = 0;
	for (s32 i = from_pos; i < src->length + 1; i++) {
		if (string_uchar(*(src->bytes + i)) ==
			string_uchar(*(find->bytes + found_chars))) {
			found_chars += 1;
			if (found_chars >= find->length)
				return (1 + i - found_chars);
		} else {
...
s64 string_to_s64(struct string *s1) { return (atol(s1->bytes)); }

s32 string_to_s32(struct string *s1) { return (atoi(s1->bytes)); }

f64 string_to_f64(struct string *s1) { return (atof(s1->bytes)); }

f32 string_to_f32(struct string *s1) { return (strtof(s1->bytes, 0)); }
...
struct string *string_replace(struct string *src, struct string *find,
	...
	do {
		...
		if (p == -1) {
			string_append_bytes(result, src->bytes + offset,
								src->length - offset);
			return result;
		}
		if (p > 0)
...
u64 hash_into_u64(u8 *data, u32 length) {
	...
	for (u32 i = 0; i < length; i++) {
		hash += ((i + salt + data[i] * last) ^ (data[i] * hash)) +
				(hash << (i % 31));
		last = data[i];
	}
...
u64 char_hash(char *s) { return (hash_into_u64(s, strlen(s))); }

u64 u64_hash(u64 v) { return (hash_into_u64((void *)&v, 8)); }
...
void *list_pop(struct list *l) { return list_delete_item(l, l->last); }

void *list_shift(struct list *l) { return list_delete_item(l, l->first); }
