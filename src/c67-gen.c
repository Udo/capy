...
#define ALWAYS_ASSERT(x)                                                       \
	do {                                                                       \
		if (!(x))                                                              \
			tcc_error("internal compiler error file at %s:%d", __FILE__,       \
					  __LINE__);                                               \
	} while (0)
...
void C67_asm(char *s, int a, int b, int c) {
	...
	if (strstr(s, "MVKL") == s) {
		...
		C67_g((C67_map_regn(b) << 23) | (((a >> 16) & 0xffff) << 7) |
		...
		C67_g((C67_map_regn(a) << 23) | // src
			  ...
			  (7 << 4) | // ldst 3=STB, 5=STH 5, 7=STW, 6=LDW 4=LDH 2=LDB 0=LDHU
						 // 1=LDBU
			  (1 << 2) | // opcode
		...
		C67_g((C67_map_regn(a) << 23) | // src
			  ...
			  (0 << 8) | // r (LDDW bit 0)
			  (0 << 7) | // y D1/D2 A side
			  (3 << 4) | // ldst 3=STB, 5=STH 5, 7=STW, 6=LDW 4=LDH 2=LDB 0=LDHU
						 // 1=LDBU
			  (1 << 2) | // opcode
			  (C67_map_regs(a) << 1) | // side of src
			  (0 << 0));			   // parallel
		...
		C67_g((C67_map_regn(a) << 23) | // src
			  ...
			  (0 << 8) | // r (LDDW bit 0)
			  (0 << 7) | // y D1/D2 A side
			  (5 << 4) | // ldst 3=STB, 5=STH 5, 7=STW, 6=LDW 4=LDH 2=LDB 0=LDHU
						 // 1=LDBU
			  (1 << 2) | // opcode
			  (C67_map_regs(a) << 1) | // side of src
			  (0 << 0));			   // parallel
		...
		C67_g((C67_map_regn(a) << 23) | // dst
			  ...
			  (0 << 8) |				// r (LDDW bit 0)
			  (1 << 7) |				// y D1/D2  B side
			  (6 << 4) | // ldst 3=STB, 5=STH 5, 7=STW, 6=LDW 4=LDH 2=LDB 0=LDHU
						 // 1=LDBU
			  (1 << 2) | // opcode
		...
		C67_g((C67_map_regn(a) << 23) | // dst
			  ...
			  (0 << 8) | // r (LDDW bit 0)
			  (0 << 7) | // y D1/D2  A side
			  (4 << 4) | // ldst 3=STB, 5=STH 5, 7=STW, 6=LDW 4=LDH 2=LDB 0=LDHU
						 // 1=LDBU
			  (1 << 2) | // opcode
			  (C67_map_regs(a) << 1) | // side of dst
			  (0 << 0));			   // parallel
		...
		C67_g((C67_map_regn(a) << 23) | // dst
			  ...
			  (0 << 8) | // r (LDDW bit 0)
			  (0 << 7) | // y D1/D2  A side
			  (2 << 4) | // ldst 3=STB, 5=STH 5, 7=STW, 6=LDW 4=LDH 2=LDB 0=LDHU
						 // 1=LDBU
			  (1 << 2) | // opcode
			  (C67_map_regs(a) << 1) | // side of dst
			  (0 << 0));			   // parallel
		...
		C67_g((C67_map_regn(a) << 23) | // dst
			  ...
			  (0 << 8) | // r (LDDW bit 0)
			  (0 << 7) | // y D1/D2  A side
			  (0 << 4) | // ldst 3=STB, 5=STH 5, 7=STW, 6=LDW 4=LDH 2=LDB 0=LDHU
						 // 1=LDBU
			  (1 << 2) | // opcode
			  (C67_map_regs(a) << 1) | // side of dst
			  (0 << 0));			   // parallel
		...
		C67_g((C67_map_regn(a) << 23) | // dst
			  ...
			  (0 << 8) | // r (LDDW bit 0)
			  (0 << 7) | // y D1/D2  A side
			  (1 << 4) | // ldst 3=STB, 5=STH 5, 7=STW, 6=LDW 4=LDH 2=LDB 0=LDHU
						 // 1=LDBU
			  (1 << 2) | // opcode
			  (C67_map_regs(a) << 1) | // side of dst
			  (0 << 0));			   // parallel
...
void C67_MVKL(int r, int fc) { C67_asm("MVKL.", fc, r, 0); }

void C67_MVKH(int r, int fc) { C67_asm("MVKH.", fc, r, 0); }
...
void C67_CMPLT(int s1, int s2, int dst) { C67_asm("CMPLT.L1", s1, s2, dst); }

void C67_CMPGT(int s1, int s2, int dst) { C67_asm("CMPGT.L1", s1, s2, dst); }

void C67_CMPEQ(int s1, int s2, int dst) { C67_asm("CMPEQ.L1", s1, s2, dst); }

void C67_CMPLTU(int s1, int s2, int dst) { C67_asm("CMPLTU.L1", s1, s2, dst); }

void C67_CMPGTU(int s1, int s2, int dst) { C67_asm("CMPGTU.L1", s1, s2, dst); }
...
void C67_NOP(int n) { C67_asm("NOP", n, 0, 0); }
...
void C67_MV(int r, int v) { C67_asm("MV.L", 0, r, v); }

void C67_DPTRUNC(int r, int v) { C67_asm("DPTRUNC.L", 0, r, v); }

void C67_SPTRUNC(int r, int v) { C67_asm("SPTRUNC.L", 0, r, v); }

void C67_INTSP(int r, int v) { C67_asm("INTSP.L", 0, r, v); }

void C67_INTDP(int r, int v) { C67_asm("INTDP.L", 0, r, v); }

void C67_INTSPU(int r, int v) { C67_asm("INTSPU.L", 0, r, v); }

void C67_INTDPU(int r, int v) { C67_asm("INTDPU.L", 0, r, v); }

void C67_SPDP(int r, int v) { C67_asm("SPDP.L", 0, r, v); }
...
void C67_ADD(int r, int v) { C67_asm("ADD.L", v, r, v); }

void C67_SUB(int r, int v) { C67_asm("SUB.L", v, r, v); }

void C67_AND(int r, int v) { C67_asm("AND.L", v, r, v); }

void C67_OR(int r, int v) { C67_asm("OR.L", v, r, v); }

void C67_XOR(int r, int v) { C67_asm("XOR.L", v, r, v); }

void C67_ADDSP(int r, int v) { C67_asm("ADDSP.L", v, r, v); }

void C67_SUBSP(int r, int v) { C67_asm("SUBSP.L", v, r, v); }

void C67_MPYSP(int r, int v) { C67_asm("MPYSP.M", v, r, v); }

void C67_ADDDP(int r, int v) { C67_asm("ADDDP.L", v, r, v); }

void C67_SUBDP(int r, int v) { C67_asm("SUBDP.L", v, r, v); }

void C67_MPYDP(int r, int v) { C67_asm("MPYDP.M", v, r, v); }

void C67_MPYI(int r, int v) { C67_asm("MPYI.M", v, r, v); }

void C67_SHL(int r, int v) { C67_asm("SHL.S", r, v, v); }

void C67_SHRU(int r, int v) { C67_asm("SHRU.S", r, v, v); }

void C67_SHR(int r, int v) { C67_asm("SHR.S", r, v, v); }
	...
	if ((vtop->r & (VT_VALMASK | VT_LVAL)) == VT_CONST) {
		...
		if (vtop->r & VT_SYM) {
			...
			if (is_jmp) {
				...
				sym =
					get_sym_ref(&char_pointer_type, cur_text_section, ind + 12,
...
void gfunc_epilog(void) {
	{
		int local = (-loc + 7) &
					-8; // stack must stay aligned to 8 bytes for LDDW instr
		C67_POP(C67_B3);
