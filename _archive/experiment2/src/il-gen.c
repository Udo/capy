...
static void init_outfile(void) {
	if (!il_outfile) {
		il_outfile = stdout;
		fprintf(il_outfile, ".assembly extern mscorlib\n"
							"{\n"
							".ver 1:0:2411:0\n"
							"}\n\n");
	}
...
void gfunc_start(GFuncContext *c, int func_call) { c->func_call = func_call; }
...
void gfunc_prolog(int t) {
	...
	while ((sym = sym->next) != NULL) {
		u = sym->t;
		sym_push(sym->v & ~SYM_FIELD, u, VT_LOCAL | lvalue_type(sym->type.t),
				 addr);
		addr++;
	}
}
