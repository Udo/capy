...
static void strcat_vprintf(char *buf, int buf_size, const char *fmt,
						   va_list ap) {
	int len;
	len = strlen(buf);
...
static void error1(int mode, const char *fmt, va_list ap) {
	...
	if (f) {
		...
		strcat_printf(buf, sizeof(buf),
					  ANSI_FG_BLUE "%s" ANSI_RESET ":" ANSI_FG_CYAN
								   "%d" ANSI_RESET " ",
...
no_file:
	...
	if (mode == ERROR_WARN)
		strcat_printf(buf, sizeof(buf), ANSI_FG_YELLOW "warning: " ANSI_RESET);
	else
		strcat_printf(buf, sizeof(buf), ANSI_FG_RED "error: " ANSI_RESET);
...
LIBTCCAPI TCCErrorFunc tcc_get_error_func(TCCState *s) { return s->error_func; }

LIBTCCAPI void *tcc_get_error_opaque(TCCState *s) { return s->error_opaque; }
...
#undef gnu_ext
	...
	s->cversion = 201112;		   /* default unless -std=c99 is supplied */
	...
	tcc_define_symbol(s, "u8", "unsigned char");
	tcc_define_symbol(s, "s8", "char");
	tcc_define_symbol(s, "b8", "char");
	tcc_define_symbol(s, "u16", "unsigned short");
	tcc_define_symbol(s, "s16", "short");
	tcc_define_symbol(s, "u32", "unsigned int");
	tcc_define_symbol(s, "s32", "int");
	tcc_define_symbol(s, "f32", "float");
	tcc_define_symbol(s, "u64", "unsigned long");
	tcc_define_symbol(s, "s64", "long");
	...
	/* used by _fd_def.h */
	tcc_define_symbol(s, "__builtin_bzero(p, ignored_size)",
					  "bzero(p, sizeof(*(p)))");
...
ST_FUNC int tcc_add_crt(TCCState *s1, const char *filename) {
	if (-1 == tcc_add_library_internal(s1, "%s/%s", filename, 0, s1->crt_paths,
									   s1->nb_crt_paths))
		tcc_error_noabort("file '%s' not found", filename);
	return 0;
}
...
LIBTCCAPI int tcc_add_library(TCCState *s, const char *libraryname) {
	...
	while (*pp) {
		if (0 == tcc_add_library_internal(s, *pp, libraryname, flags,
										  s->library_paths,
										  s->nb_library_paths))
			return 0;
		++pp;
...
/* read list file */
static void args_parser_listfile(TCCState *s, const char *filename, int optind,
