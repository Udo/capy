...
ST_FUNC void load(int r, SValue *sv) {
	...
	if (svr == (VT_CONST | VT_LVAL | VT_SYM)) {
		...
		if (IS_FREG(r))
			...
			arm64_ldrx(!(svtt & VT_UNSIGNED), arm64_type_size(svtt), intr(r),
					   30, arm64_check_offset(1, arm64_type_size(svtt), svcul));
		return;
	}
	...
	if (svr == VT_CONST) {
		if ((svtt & VT_BTYPE) != VT_VOID)
			arm64_movimm(intr(r), arm64_type_size(svtt) == 3 ? sv->c.i
															 : (uint32_t)svcul);
		return;
	}
...
ST_FUNC void gfunc_return(CType *func_type) {
	...
	case 16:
		if ((func_type->t & VT_BTYPE) == VT_STRUCT) {
			...
			gv(RC_R(0));
			for (j = 0; j < n; j++)
				o(0x3d400000 | (sz & 16) << 19 | -(sz & 8) << 27 |
				  (sz & 4) << 29 | j |
				  j << 10); // ldr ([sdq])(*),[x0,#(j * sz)]
		} else
			gv(RC_FRET);
		break;
	default:
...
static int arm64_gen_opic(int op, uint32_t l, int rev, uint64_t val, uint32_t x,
						  uint32_t a) {
	if (op == '-' && !rev) {
		val = -val;
		op = '+';
	...
	case TOK_SHR: {
		...
		if (!val) {
			...
			o(0x13000000 | (op == TOK_SHR) << 30 | l << 31 | l << 22 | x |
			  a << 5 | val << 16 | (n - 1) << 10); // lsr/asr
		return 1;
	}
	}
	return 0;
...
ST_FUNC void gen_cvt_csti(int t) {
	...
	o(0x13001c00 | ((t & VT_BTYPE) == VT_SHORT) << 13 |
	  (uint32_t) !!(t & VT_UNSIGNED) << 30 | r |
	  r << 5); // [su]xt[bh] w(r),w(r)
}
