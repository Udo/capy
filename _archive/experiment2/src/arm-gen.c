	...
	for (i = nb_args; i--;) {
		...
		case VT_LDOUBLE:
			if (float_abi == ARM_HARD_FLOAT) {
				...
				if (is_float(vtop[-i].type.t) ||
					...
					if (start_vfpreg >= 0) {
						add_param_plan(plan,
									   is_hfa ? VFP_STRUCT_CLASS : VFP_CLASS,
			...
			if (ncrn + size / 4 <= 4 || (ncrn < 4 && start_vfpreg != -1)) {
				 ...
				 * stack. As such, it can be of either class: it would either be
				 * the last of CORE_STRUCT_CLASS or the first of STACK_CLASS. */
		...
		default:
			if (ncrn < 4) {
				...
				add_param_plan(plan, CORE_CLASS, ncrn, ncrn + is_long,
							   &vtop[-i]);
				ncrn += 1 + is_long;
				continue;
			}
...
static int copy_params(int nb_args, struct plan *plan, int todo) {
	...
	/* Several constraints require parameters to be copied in a specific order:
	   ...
	   because a structure can lie partly in core registers and partly on the
	   stack;
...
again:
		...
		for (pplan = plan->clsplans[i]; pplan; pplan = pplan->prev) {
			...
			case VFP_STRUCT_CLASS:
				if ((pplan->sval->type.t & VT_BTYPE) == VT_STRUCT) {
					...
					if (is_float(pplan->sval->type.t)) {
						...
						o(0xED2D0100 | r |
						  (size >> 2)); /* some kind of vpush for FPA */
#endif
					} else {
			...
			case VFP_CLASS:
				...
				if (pplan->start &
					...
					vtop->r = VT_CONST; /* avoid being saved on stack by gv for
										   next float */
				}
				break;
	...
	if (todo) {
		...
		for (pplan = plan->clsplans[CORE_STRUCT_CLASS]; pplan;
		   ...
		   structure can occupy more than 2 registers. Thus, we need to push on
		   the value stack some fake parameter to have on SValue for each
		   registers used by a structure (r2 is not used). */
...
void gfunc_call(int nb_args) {
	...
	/* cannot let cpu flags if other instruction are generated. Also avoid
	   leaving VT_JMP anywhere except on the top of the stack because it would
	   complicate the code generator. */
	r = vtop->r & VT_VALMASK;
