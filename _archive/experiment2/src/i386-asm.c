...
static void parse_operand(TCCState *s1, Operand *op) {
	...
	if (tok == '%') {
		...
		if (tok >= TOK_ASM_al && tok <= TOK_ASM_db7) {
			reg = tok - TOK_ASM_al;
			op->type =
				1 << (reg >> 3); /* WARNING: do not change constant order */
			op->reg = reg & 7;
...
static void maybe_print_stats(void) {
	...
	/* print stats about opcodes */
	{
		...
		printf("size=%d nb=%d f0=%d f1=%d f2=%d f3=%d\n",
			   ...
			   (int)sizeof(asm_instrs) / (int)sizeof(ASMInstr), freq[0],
...
ST_FUNC void asm_opcode(TCCState *s1, int opcode) {
	...
	for (;;) {
		...
		if (tok == ':') {
			...
			if (!(pop->type & OP_EA)) {
				tcc_error(
					"segment prefix must be followed by memory reference");
			}
		}
		pop++;
		nb_ops++;
...
again:
	...
	modrm_found:
		...
		for (i = 0; i < nb_ops; i++) {
			int t = op_type[i];
			if (i != modrm_index && (t & (OP_REG | OP_MMX | OP_SSE | OP_CR |
										  OP_TR | OP_DB | OP_SEG))) {
				modreg_index = i;
				break;
			}
	...
	for (i = 0; i < nb_ops; i++) {
		...
		if (v & (OP_IM8 | OP_IM16 | OP_IM32 | OP_IM64 | OP_IM8S | OP_ADDR)) {
		   ...
		   at the op size */
			if ((v | OP_IM8 | OP_IM64) ==
				(OP_IM8 | OP_IM16 | OP_IM32 | OP_IM64)) {
				if (s == 0)
					v = OP_IM8;
				else if (s == 1)
					v = OP_IM16;
			...
			if (v & (OP_IM8 | OP_IM8S)) {
				...
				gen_expr64(&ops[i].e);
#endif
			} else if (pa->op_type[i] == OPT_DISP ||
					   pa->op_type[i] == OPT_DISP8) {
				gen_disp32(&ops[i].e);
			} else {
				gen_expr32(&ops[i].e);
			}
	...
	*pout_reg = -1;
	...
	for (i = 0; i < nb_operands; i++) {
		...
		printf("%%%d [%s]: \"%s\" r=0x%04x reg=%d\n", j,
			   op->id ? get_tok_str(op->id, NULL) : "", op->constraint,
			   op->vt->r, op->reg);
	}
	if (*pout_reg >= 0)
		printf("out_reg=%d\n", *pout_reg);
