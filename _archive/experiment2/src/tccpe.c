...
static DWORD umin(DWORD a, DWORD b) { return a < b ? a : b; }

static DWORD umax(DWORD a, DWORD b) { return a < b ? b : a; }
...
#define CHARACTERISTICS_DLL 0x230F
	...
	if (NULL == pf.op) {
		tcc_error_noabort("could not write '%s': %s", pe->filename,
						  strerror(errno));
		return -1;
	}
	...
	for (i = 0; i < pe->sec_count; ++i) {
		...
		case sec_reloc:
			pe_set_datadir(&pe_header, IMAGE_DIRECTORY_ENTRY_BASERELOC, addr,
						   size);
			break;

		case sec_rsrc:
			pe_set_datadir(&pe_header, IMAGE_DIRECTORY_ENTRY_RESOURCE, addr,
						   size);
			break;

		case sec_pdata:
			pe_set_datadir(&pe_header, IMAGE_DIRECTORY_ENTRY_EXCEPTION, addr,
						   size);
			break;
		}
		...
		pe_header.opthdr.SizeOfImage = umax(pe_virtual_align(pe, size + addr),
	...
	for (i = 0; i < pe->imp_count; ++i) {
		...
		for (k = 0, n = p->sym_count; k <= n; ++k) {
			if (k < n) {
				...
				ElfW(Sym) *org_sym =
					(ElfW(Sym) *)symtab_section->data + iat_index;
				const char *name =
					(char *)pe->s1->dynsymtab_section->link->data +
					imp_sym->st_name;
				int ordinal;
				...
				if (dllref)
					v = 0,
					ordinal = imp_sym->st_value; /* ordinal from pe_load_def */
				else
					ordinal = 0,
					v = imp_sym->st_value; /* address from tcc_add_symbol() */
				...
				if (pe->type == PE_RUN) {
					...
					if (!v)
						tcc_error_noabort("could not resolve symbol '%s'",
										  name);
				} else
#endif
					if (ordinal) {
					...
					section_ptr_add(pe->thunk,
									sizeof(WORD)); /* hint, not used */
...
static void pe_build_exports(struct pe_info *pe) {
	...
	for (ord = 0; ord < sym_count; ++ord) {
		...
		put_elf_reloc(symtab_section, pe->thunk, func_o, R_XXX_RELATIVE,
					  sym_index);
	...
	for (sym_index = 1; sym_index < sym_end; ++sym_index) {
			...
			if (type == STT_FUNC) {
				...
				if (offset) {
					...
					*(DWORD *)(p + 1) = (DWORD)-4;
					...
					is->iat_index =
						put_elf_sym(symtab_section, 0, sizeof(DWORD),
		...
		not_found:
			if (ELFW(ST_BIND)(sym->st_info) == STB_WEAK)
				...
				continue;
			tcc_error_noabort("undefined symbol [PE executable] '%s'%s", name,
...
static void pe_print_section(FILE *f, Section *s) {
	...
	for (i = 0; i < l;) {
		...
		if (s->sh_type == SHT_SYMTAB) {
			...
			fprintf(
				f, "  %04X  %04X  %04X   %02X    %02X    %02X   %04X  \"%s\"",
...
static unsigned pe_add_uwwind_info(TCCState *s1) {
	...
	if (0 == s1->uw_offs) {
		/* As our functions all have the same stackframe, we use one entry for
		 * all
		 */
		static const unsigned char uw_info[] = {
			...
			0x05, // UBYTE: 4 Frame Register (rbp), UBYTE: 4 Frame Register
				  // offset (scaled)
