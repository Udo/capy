...
ST_INLN int is_basic_type(int t) {
	int bt = t & VT_BTYPE;
	return bt == VT_LDOUBLE || bt == VT_DOUBLE || bt == VT_FLOAT ||
		   bt == VT_QFLOAT || bt == VT_BYTE || bt == VT_BOOL ||
...
long int capy_hash_string(char* data, int length)
{
	...
	for (int i = 0; i < length; i++) {
		hash += ((i + salt + data[i]*last) ^ (data[i]*hash)) + (hash <<
(i % 31)); last = data[i];
	}
...
ST_FUNC void tcc_debug_start(TCCState *s1) {
	if (s1->do_debug) {
		...
		put_stabs_r(s1, buf, N_SO, 0, 0, text_section->data_offset,
...
static void tcc_debug_stabs(TCCState *s1, const char *str, int type,
	...
	if (debug_info) {
		debug_info->sym = (struct debug_sym *)tcc_realloc(
			debug_info->sym,
			sizeof(struct debug_sym) * (debug_info->n_sym + 1));
...
static void tcc_get_debug_info(TCCState *s1, Sym *s, CString *result) {
	...
	if ((type & VT_BTYPE) == VT_STRUCT) {
		...
		if (debug_type == -1) {
			...
			debug_hash = (struct debug_hash *)tcc_realloc(
...
static void tcc_debug_extern_sym(TCCState *s1, Sym *sym, int sh_num,
								 int sym_bind) {
	...
	if (sym_bind == STB_GLOBAL)
		...
		tcc_debug_stabs(
			s1, str.data,
			(sym->type.t & VT_STATIC) && data_section == s ? N_STSYM : N_LCSYM,
			0, s, sym->c);
	cstr_free(&str);
}
...
static BufferedFile *put_new_file(TCCState *s1) {
	...
	if (f->filename[0] == ':')
		f = f->prev;
	if (f && new_file) {
		put_stabs_r(s1, f->filename, N_SOL, 0, 0, ind, text_section,
					section_sym);
		new_file = last_line_num = 0;
	}
	return f;
...
#endif
	parse_flags =
		PARSE_FLAG_PREPROCESS | PARSE_FLAG_TOK_NUM | PARSE_FLAG_TOK_STR;
	next();
	decl(VT_CONST);
...
ST_FUNC void put_extern_sym2(Sym *sym, int sh_num, addr_t value,
	...
	if (!sym->c) {
		...
		if (tcc_state->do_debug && sym_type != STT_FUNC &&
...
/* find an identifier */
ST_INLN Sym *sym_or_struct_find(int v) {
	v -= TOK_IDENT;
	if ((unsigned)v >= (unsigned)(tok_ident - TOK_IDENT))
		return NULL;
	...
	if (!(v & SYM_FIELD) && (v & ~SYM_STRUCT) < SYM_FIRST_ANOM) {
		...
		*ps = s;
		...
		if (s->prev_tok && sym_scope(s->prev_tok) == s->sym_scope)
			tcc_error("cannot redeclare '" ANSI_FG_CYAN "%s" ANSI_RESET "'",
					  get_tok_str(v & ~SYM_STRUCT, NULL));
	}
	return s;
}
...
ST_FUNC Sym *global_identifier_push(int v, int t, int c) {
	...
	if (v < SYM_FIRST_ANOM) {
		...
		*ps = s;
		...
		// printf(" ident[%s] ", get_tok_str(v, 0));
	}
	return s;
}
...
static void patch_type(Sym *sym, CType *type) {
	...
	if (!(type->t & VT_EXTERN) || IS_ENUM_VAL(sym->type.t)) {
		if (!(sym->type.t & VT_EXTERN)) {
			...
			type_to_str(tbuf2, sizeof(tbuf1), type, NULL);
			tcc_error("type error, cannot redefine " ANSI_FG_CYAN
					  "%s" ANSI_RESET "(%s) to " ANSI_FG_CYAN "%s" ANSI_RESET
					  "(%s)",
					  get_tok_str(sym->v, NULL), tbuf1,
					  get_tok_str(sym->v, NULL), tbuf2);
		}
	...
	if (!is_compatible_types(&sym->type, type)) {
		tcc_error("incompatible types for redefinition of '" ANSI_FG_CYAN
				  "%s" ANSI_RESET "'",
				  get_tok_str(sym->v, NULL));
		...
		if ((type->t & VT_STATIC) &&
			...
			tcc_warning(
				"static storage ignored for redefinition of '" ANSI_FG_CYAN
				"%s" ANSI_RESET "'",
				get_tok_str(sym->v, NULL));
		...
		if ((type->t ^ sym->type.t) & VT_STATIC)
			tcc_warning("storage mismatch for redefinition of '" ANSI_FG_CYAN
						"%s" ANSI_RESET "'",
						get_tok_str(sym->v, NULL));
	}
}
...
static void patch_storage(Sym *sym, AttributeDef *ad, CType *type) {
	...
	if (sym->a.dllimport != ad->a.dllimport)
		tcc_error("incompatible dll linkage for redefinition of '" ANSI_FG_CYAN
				  "%s" ANSI_RESET "'",
				  get_tok_str(sym->v, NULL));
#endif
...
static void add_local_bounds(Sym *s, Sym *e) {
	for (; s != e; s = s->prev) {
		...
		if ((s->type.t & VT_ARRAY) || (s->type.t & VT_BTYPE) == VT_STRUCT ||
			...
			addr_t *bounds_ptr =
				section_ptr_add(lbounds_section, 2 * sizeof(addr_t));
			bounds_ptr[0] = s->c;
			bounds_ptr[1] = size;
		}
	}
...
ST_FUNC int gv(int rc) {
	...
	if (vtop->type.t & VT_BITFIELD) {
		...
		r2_ok =
			!rc2 || ((vtop->r2 < VT_CONST) && (reg_classes[vtop->r2] & rc2));
					...
					type_incompatibility_warning(
						...
						op == '?'
							? "pointer type mismatch in conditional expression "
							  ...
							  "' and '" ANSI_FG_CYAN "%s" ANSI_RESET "')"
							: "pointer type mismatch in "
							  "comparison('" ANSI_FG_CYAN "%s" ANSI_RESET
							  "' and '" ANSI_FG_CYAN "%s" ANSI_RESET "')");
			}
			if (op == '?') {
				...
				if ((~pointed_type(&type)->t & (VT_CONSTANT | VT_VOLATILE)) &
					...
					type.ref =
						sym_push(SYM_FIELD, &type.ref->type, 0, type.ref->c);
					copied = 1;
				...
				if (pt1->t & VT_ARRAY && pt2->t & VT_ARRAY &&
					...
					if (!copied)
						type.ref = sym_push(SYM_FIELD, &type.ref->type, 0,
											type.ref->c);
		...
		if (c) {
			...
			if (df) {
				...
				if (sf)
					...
					vtop->c.i =
						((uint32_t)vtop->c.i | -(vtop->c.i & 0x80000000));
...
static void cast_error(CType *st, CType *dt) {
	type_incompatibility_error(st, dt,
							   "cannot convert '" ANSI_FG_CYAN "%s" ANSI_RESET
...
ST_FUNC void vstore(void) {
	...
	if (sbt == VT_STRUCT) {
		...
		if (dbt == VT_BOOL) {
			...
			vtop[-1].type.t =
				(vtop[-1].type.t & ~VT_BTYPE) | (VT_BYTE | VT_UNSIGNED);
		}
	...
	while (tok != ')') {
		...
		case TOK_CLEANUP2: {
			...
			if (!s) {
				...
				tcc_error("'" ANSI_FG_CYAN "%s" ANSI_RESET "' is not callable",
						  get_tok_str(tok, &tokc));
			ad->cleanup_func = s;
			next();
			skip(')');
		...
		default: {
			...
			if (tcc_state->warn_unsupported)
				tcc_warning("'" ANSI_FG_CYAN "%s" ANSI_RESET
							"' attribute ignored",
							atr_name);
			/* skip parameters */
			if (tok == '(') {
				...
				ad->a.tag =
					(s32)hash_into_u64((char *)atr_name, strlen(atr_name));
				// printf("tag [@%s] : %p\n", atr_name, ad->a.tag);
			}
			break;
		}
...
static void struct_layout(CType *type, AttributeDef *ad) {
	...
	for (f = type->ref->next; f; f = f->next) {
		...
		if (f->type.t & VT_BITFIELD) {
			printf(" pos %-2d bits %-2d", BIT_POS(f->type.t),
				   BIT_SIZE(f->type.t));
		}
		printf("\n");
#endif
	...
	for (f = type->ref->next; f; f = f->next) {
		...
		if (px + bit_size <= s * 8 && cx + s <= c) {
			...
			printf("FIX field %s offset %-2d size %-2d align %-2d "
				   "pos %-2d bits %-2d\n",
				   get_tok_str(f->v & ~SYM_FIELD, NULL), cx, s, align, px,
				   bit_size);
#endif
		} else {
...
static void struct_decl(CType *type, int u) {
	...
	if (tok != '{') {
		...
		if (s && (s->sym_scope == local_scope || tok != '{')) {
			...
			goto do_decl;
			// tcc_error("declaration error, cannot redefine '" ANSI_FG_CYAN
			// "%s" ANSI_RESET "'", get_tok_str(v, NULL));
		}
	} else {
		if (extended_enum)
			extended_enum = false;
...
do_decl:
	...
	if (tok == '{') {
		...
		if (u == VT_ENUM) {
			...
			gen_code_values = string_create_with_chars(
				"s32 "); // todo: we should do something about the actual
						 // storage type of enums, right?
			if (extended_enum) {
			...
			for (;;) {
				v = tok;
				if (v < TOK_UIDENT)
					tcc_error("identifier expected but '" ANSI_FG_CYAN
							  "%s" ANSI_RESET "' found",
							  get_tok_str(tok, 0));
				...
				if (extended_enum) {
					...
					string_append_chars(gen_code_names,
										(char *)get_tok_str(v, NULL));
					string_append_chars(gen_code_names, "\", ");
				}

				ss = sym_find(v);
				if (ss && !local_stack)
					tcc_error("cannot redefine enumerator '" ANSI_FG_CYAN
							  "%s" ANSI_RESET "'",
							  get_tok_str(v, NULL));
			...
			while (tok != '}') {
				...
				while (1) {
					if (flexible)
						tcc_error("flexible array member '" ANSI_FG_CYAN
								  "%s" ANSI_RESET
					...
					if (tok != ':') {
						...
						if (type_size(&type1, &align) < 0) {
							if ((u == VT_STRUCT) && (type1.t & VT_ARRAY) && c)
								...
								tcc_error("field '" ANSI_FG_CYAN "%s" ANSI_RESET
						...
						if ((type1.t & VT_BTYPE) == VT_FUNC ||
							...
							(type1.t & VT_STORAGE))
							tcc_error("invalid type for '" ANSI_FG_CYAN
									  "%s" ANSI_RESET "'",
									  get_tok_str(v, NULL));
					}
					if (tok == ':') {
						...
						if (bit_size < 0)
							tcc_error(
								"negative width in bit-field '" ANSI_FG_CYAN
								"%s" ANSI_RESET "'",
								get_tok_str(v, NULL));
						if (v && bit_size == 0)
							tcc_error("zero width for bit-field '" ANSI_FG_CYAN
									  "%s" ANSI_RESET "'",
									  get_tok_str(v, NULL));
						parse_attribute(&ad1);
					}
					...
					if (bit_size >= 0) {
						...
						bsize = size * 8;
						if (bit_size > bsize) {
							tcc_error("width of '" ANSI_FG_CYAN "%s" ANSI_RESET
									  "' exceeds its type",
									  get_tok_str(v, NULL));
						} else if (bit_size == bsize && !ad.a.packed &&
								   !ad1.a.packed) {
					...
					if (v == 0 &&
						((type1.t & VT_BTYPE) == VT_STRUCT || bit_size >= 0)) {
						v = anon_sym++;
					}
					if (v) {
						if (extended_enum) {
 ...
 */
static int parse_btype(CType *type, AttributeDef *ad) {
	int t, u, bt, st, type_found, typespec_found, g, n, p_tok;
	Sym *s;
	CType type1;
	...
	while (1) {
		...
		basic_type1:
			if (u == VT_SHORT || u == VT_LONG) {
				...
				tmbt:
					tcc_error("cannot redefine type/def for <%s>",
							  get_tok_str(p_tok, NULL));
				}
				st = u;
			} else {
		...
		default:
			...
			if (!s) {
				Sym *s3 = struct_find(tok);
				if (s3) {
					struct_decl(&type1, VT_STRUCT);
					goto basic_type2;
					// tcc_warning("symbol_not_found", "");
				}
			}
...
static CType *type_decl(CType *type, AttributeDef *ad, int *v, int td) {
	...
	abstract:
		if (!(td & TYPE_ABSTRACT))
			tcc_error("identifier expected but '" ANSI_FG_CYAN "%s" ANSI_RESET
					  "' found",
					  get_tok_str(tok, 0));
	...
	/*
	{
		char tbuf1[256];
		type_to_str(tbuf1, sizeof(tbuf1), type, NULL);
		printf("DEBUG: type %s sym=%s t=%i\n", tbuf1, get_tok_str(v,0),
	type_to_id(type));
	}
	*/

	/*		//if(fnattr.a.tag == TAG_HASH_FN)
		{
			char tbuf1[256];
		type_to_str(tbuf1, sizeof(tbuf1), type, NULL);
			printf("fn define: "ANSI_FG_CYAN "%s" ANSI_RESET "(%s)\n",
				get_tok_str(type -> ref, NULL), tbuf1);
		}
  */
...
static void gfunc_param_typed(Sym *func, Sym *arg, CallSite* cs) {
	int func_type;
	CType type;
...
ST_FUNC void unary(void) {
	...
	memset(&call_site, 0, sizeof(call_site)); // do we really need this?
	call_site.funcname = "";
	/* generate line number info */
	if (tcc_state->do_debug)
...
tok_next:
	...
	case TOK_GENERIC: {
		...
		if (!str) {
			...
			tcc_error("type '" ANSI_FG_CYAN "%s" ANSI_RESET
					  "' does not match any association",
					  buf);
		}
		begin_macro(str, 1);
		next();
	...
	tok_identifier:
		...
		// if (!s)
		//	s = struct_find(t);
		if (!call_site.is_capy_func && (!s || IS_ASM_SYM(s))) {
			const char *name = get_tok_str(t, NULL);
			if (tok != '(') {
				tcc_error("'" ANSI_FG_CYAN "%s" ANSI_RESET "' is undeclared",
						  name);
			}
		...
		/* Point to s as backpointer (even without r&VT_SYM).
				...
				regvars.  */
		vtop->sym = s;
		call_site.vtop_placeholder = vtop;
	...
	while (1) {
		if (tok == TOK_INC || tok == TOK_DEC) {
			...
			/* function call  */
			// printf(" call:%s ", call_site.funcname);
			if ((vtop->type.t & VT_BTYPE) != VT_FUNC) {
				...
				error_func:
					tcc_error("function pointer expected, '%s' found", get_tok_str(tok, 0));
				}
			} else {
			...
			if (ret_nregs < 0) {
				...
				for (r = ret.r + ret_nregs + !ret_nregs; r-- > ret.r;) {
					...
					vtop->r2 =
						ret.r2; /* Loop only happens when r2 is VT_CONST */
				}
	...
	if (tok == '?') {
		...
		if (!combine_types(&type, &sv, vtop, '?'))
			type_incompatibility_error(
				&sv.type, &vtop->type,
				"type mismatch in conditional expression ('" ANSI_FG_CYAN
				"%s" ANSI_RESET "' and '" ANSI_FG_CYAN "%s" ANSI_RESET "')");
...
static void check_func_return(void) {
	...
	if (!strcmp(funcname, "main") && (func_vt.t & VT_BTYPE) == VT_INT) {
		...
		tcc_warning("function might return no value: '" ANSI_FG_CYAN
					"%s" ANSI_RESET "'",
					funcname);
	}
}
		...
		if (tok == ':' && t >= TOK_UIDENT) {
			...
			if (s) {
				if (s->r == LABEL_DEFINED)
					tcc_error("duplicate label '" ANSI_FG_CYAN "%s" ANSI_RESET
							  "'",
	...
	if (sec) {
		...
		if ((vtop->r & (VT_SYM | VT_CONST)) == (VT_SYM | VT_CONST) &&
			...
			if (ssec->reloc) {
				...
				rel =
					(ElfW_Rel *)(ssec->reloc->data + ssec->reloc->data_offset);
				while (num_relocs--) {
					rel--;
					...
					put_elf_reloca(
						symtab_section, sec, c + rel->r_offset - esym->st_value,
			...
			if (type->t & VT_BITFIELD) {
				...
				switch (bt) {
					/* XXX: when cross-compiling we assume that each type has
					   the same representation on host and target, which is
					   likely to be wrong in the case of long double */
				case VT_BOOL:
					vtop->c.i = vtop->c.i != 0;
				case VT_BYTE:
					*(char *)ptr |= vtop->c.i;
...
static void decl_initializer_alloc(CType *type, AttributeDef *ad, int r,
	...
	if (flexible_array && flexible_array->type.ref->c > 0)
		size +=
			flexible_array->type.ref->c * pointed_size(&flexible_array->type);
	...
	if ((r & VT_VALMASK) == VT_LOCAL) {
		...
		if (bcheck) {
			...
			greloca(bounds_section, sym, bounds_section->data_offset,
					R_DATA_PTR, 0);
...
static void gen_function(Sym *sym) {
	...
	if (sym->a.aligned) {
		size_t newoff =
			section_add(cur_text_section, 0, 1 << (sym->a.aligned - 1));
	...
	while (1) {
		...
		while (1) { /* iterate thru each declaration */
...
#if 0
			{
				...
				printf("type = '" ANSI_FG_CYAN "%s" ANSI_RESET "'\n", buf);
			}
#endif
			if ((type.t & VT_BTYPE) == VT_FUNC) {
				...
				if (ad.a.tag == TAG_HASH_FN) {
					...
					const char *bare_identifier =
						get_tok_str(bare_identifier_v, 0);
					char tbuf1[256];
					char vident[256];
					 ...
					 * this is a pattern-matched function type
					 * - so we can later compare the type signature at calling
					 * sites
					 * - make a decision about what to do with the return type
					 *   - maybe ignore, make it not part of the signature?
					 *   - or try to infer the return type from the calling
					 * site?
					 * - we should convenience-match basic numeric types as long
					 * as information is not lost
					 * - if no capy function matches, we should also look
					 * whether there's a C function that does
					 */
					/*
					int bare_ident_len = strlen(get_tok_str(v, 0));
					table_ident[v - TOK_IDENT]->str[bare_ident_len] = ':';
					sprintf(
				...
				if (sym->f.func_alwinl && ((type.t & (VT_EXTERN | VT_INLINE)) ==
										   (VT_EXTERN | VT_INLINE))) {
			...
			if (ad.a.dllimport || ad.a.dllexport) {
				...
				if (type.t & VT_TYPEDEF) {
					tcc_warning("'" ANSI_FG_CYAN "%s" ANSI_RESET
								"' attribute ignored for typedef",
								ad.a.dllimport
									? (ad.a.dllimport = 0, "dllimport")
				...
				if (l == VT_CMP) {
					...
					for (sym = func_sym->next; sym; sym = sym->next)
						if ((sym->v & ~SYM_FIELD) == v)
							goto found;
					tcc_error("declaration for parameter '" ANSI_FG_CYAN
							  "%s" ANSI_RESET "' but no such parameter",
							  get_tok_str(v, NULL));
				found:
					if (type.t & VT_STORAGE) /* 'register' is okay */
						tcc_error("storage class specified for '" ANSI_FG_CYAN
								  "%s" ANSI_RESET "'",
								  get_tok_str(v, NULL));
					if (sym->type.t != VT_VOID)
						tcc_error("redefinition of parameter '" ANSI_FG_CYAN
								  "%s" ANSI_RESET "'",
								  get_tok_str(v, NULL));
					...
					if (sym && sym->sym_scope == local_scope) {
						if (!is_compatible_types(&sym->type, &type) ||
							...
							tcc_error(
								"incompatible redefinition of '" ANSI_FG_CYAN
								"%s" ANSI_RESET "'",
								get_tok_str(v, NULL));
						sym->type = type;
					} else {
...
static void decl(int l) { decl0(l, 0, NULL); }
