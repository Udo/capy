...
void gfunc_call(int nb_args) {
	...
	for (i = 0; i < nb_args; i++) {
		...
		if (using_regs(size))
			continue; /* arguments smaller than 8 bytes passed in registers or
				 on stack */
...
void gfunc_call(int nb_args) {
	...
	for (i = 0; i < nb_args; i++) {
		mode =
			classify_x86_64_arg(&vtop->type, &type, &size, &align, &reg_count);
	...
	if (vtop->type.ref->f.func_type !=
		...
		oad(0xb8,
			nb_sse_args < 8 ? nb_sse_args : 8); /* mov nb_sse_args, %eax */
	gcall_or_jmp(0);
	if (args_size)
		gadd_sp(args_size);
	vtop--;
...
void gfunc_prolog(Sym *func_sym) {
	...
	if (func_var) {
		...
		while ((sym = sym->next) != NULL) {
			...
			stack_arg:
				seen_stack_size =
					((seen_stack_size + align - 1) & -align) + size;
				break;
		...
		case x86_64_mode_sse:
			...
			if (sse_param_index + reg_count <= 8) {
				...
				for (i = 0; i < reg_count; ++i) {
					o(0xd60f66); /* movq */
					gen_modrm(sse_param_index, VT_LOCAL, NULL,
							  param_addr + i * 8);
					++sse_param_index;
				}
			} else {
...
void gen_opl(int op) { gen_opi(op); }
