...
typedef unsigned char 	xx_u8;
typedef char	 		xx_s8;
typedef char	 		xx_b8;
typedef unsigned short 	xx_u16;
typedef short			xx_s16;
typedef unsigned int 	xx_u32;
typedef int			 	xx_s32;
typedef float		 	xx_f32;
typedef unsigned long 	xx_u64;
typedef long			xx_s64;
typedef double		 	xx_f64;
...
void string_append_bytes(struct string* s, void* addr, s32 length)
{
	...
	s32 new_length = length + s->length;
...
{
	s32 len = s1->length < s2->length ? s1->length : s2->length;
	for(s32 i = 0; i < len; i++)
	{
		u8 c1 = *(s1->bytes + i);
		u8 c2 = *(s2->bytes + i);
...
struct string* string_ireplace(struct string* src, struct string* find, struct string* rwith)
{
	...
	s32 offset = 0;
	do {
		s32 p = string_ipos(src, find, offset);
		if(p == -1)
		{
			string_append_bytes(result, src->bytes + offset, src->length - offset);
			return result;
...
{
	struct string* result = string_create();
    FILE *file = fopen(file_name->bytes, "rb");
    if (!file)
        return result;

	s32 fsize = file_size(file_name);
	if(fsize == 0)
		return result;

	string_recap(result, fsize);
	s32 read_size = fread(result->bytes, 1, fsize, file);
    fclose(file);
...
{
    FILE *fp = fopen(file_name->bytes, "wb");
    if(!fp)
		return false;
...
{
	struct list* result = mem_calloc(sizeof(struct list), 1);
	if(!result)
...
{
	l->length += 1;
	struct list_item* new_item = mem_calloc(sizeof(struct list_item), 1);
...
void* list_delete_item(struct list* l, struct list_item* it)
{
	...
	if(it->prev)
	{
		struct list_item* pi = it->prev;
		pi->next = it->next;
	}
	if(it->next)
	{
		struct list_item* pi = it->next;
		pi->prev = it->prev;
	}
	if(l->first == it)
		l->first = it->next;
...
struct list* string_split(struct string* src, struct string* delim)
{
	...
	s32 pos = string_pos(src, delim, 0);
	s32 start = 0;
	if(pos == -1)
	{
...
{
	struct hashmap* result = mem_calloc(sizeof(struct hashmap), 1);
	if(!result)
...
void hashmap_free_buckets(struct hashmap_item* buckets, s32 count)
{
	...
	{
		struct hashmap_item* item = &buckets[i];
		if(item->hash)
		{
			item = item->next;
			while(item)
...
void hashmap_insert(struct hashmap* h, u64 hash, void* item_data, struct string* key);
void hashmap_recap(struct hashmap* h, s32 new_capacity)
{
	struct hashmap_item* old_buckets = h->buckets;
	struct hashmap_item* old_first_in_order = h->first_in_order;
	struct hashmap_item* old_last_in_order = h->last_in_order;
	h->first_in_order = 0;
	h->last_in_order = 0;
	s32 old_capacity = h->capacity;
	h->is_resizing = true;
	h->capacity = new_capacity;
	h->buckets = mem_calloc(sizeof(struct hashmap_item), new_capacity);
	if(old_buckets)
	{
		struct hashmap_item* it = old_first_in_order;
		while(it)
		{
...
{
	s32 idx = hash % h->capacity;
	struct hashmap_item* bucket = &h->buckets[idx];
	if(!bucket->hash)
		return 0;
	while(bucket)
	{
...
{
	struct hashmap_item* entry = hashmap_get_entry(h, hash);
	if(entry)
		return entry->data;
	return 0;
}
...
void hashmap_insert(struct hashmap* h, u64 hash, void* item_data, struct string* key)
{
	...
	{
		struct hashmap_item* dupe = hashmap_get_entry(h, hash);
		if(dupe)
		{
			dupe->data = item_data;
		}
		...
		{
			struct hashmap_item* new_item = mem_calloc(sizeof(struct hashmap_item), 1);
			new_item->hash = hash;
			new_item->key = key;
			new_item->data = item_data;
...
{
	u64 hash = char_hash(key);
	struct string* key_string = string_create_with_chars(key);
...
{
	struct hashmap_item* it = hashmap_get_entry(h, hash);
	if(!it)
		return 0;
...
{
	struct list* result = list_create();
	struct hashmap_item* it = hashmap_first_entry(h);
	while(it)
	{
